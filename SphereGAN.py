...
class SphereGAN(object):
    ...
    def __init__(self, sess, args):
        self.model_name = "SphereGAN"  # name for checkpoint
        self.sess = sess
    ...
    def generator(self, z, is_training=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            x = tf.reshape(x, [self.batch_size, 4, 4, ch])

            n_res = int(np.log2((self.img_size // 4)))

            for i in range(n_res) :
                x = resblock_up(x, channels=ch, is_training=is_training, sn=self.sn, scope='res_up_' + str(i))

            # x = batch_norm(x, is_training, scope='batch_norm')
            # x = relu(x)

            x = conv(x, channels=self.c_dim, kernel=3, stride=1, pad=1, sn=self.sn, scope='logit')
            x = batch_norm(x, is_training, scope='batch_norm')
            x = tanh(x)
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            n_res = int(np.log2(self.img_size) / 2)

            for i in range(n_res) :
                x = resblock_down(x, channels=ch, is_training=is_training, sn=self.sn, scope='res_down_' + str(i))

            for i in range(n_res) :
                x = resblock(x, channels=ch, is_training=is_training, sn=self.sn, scope='res_' + str(i))

            x = lrelu(x, 0.2)
            x = global_avg_pooling(x)

            x = fully_connected(x, units=self.ch * 4, sn=self.sn, scope='logit')
